...
<div class="widget">
				  ...
				  <select class="form-control" id="cityFilter">
					...
					<h1 class="page-name"><?=!empty($_GET['cat']) ? 'Dresses' : 'All Products'?></h1>

					<ol class="breadcrumb">
						<li><a href="index.php">Home</a></li>

						<li class="active"><?=!empty($_GET['cat']) ? $_GET['cat'] : 'All'?> outfits</li>
					</ol>
				</div>
			</div>
		</div>
			...
			if($_GET)
			{
				...
				if(!empty($_GET['city']))
				{
					...
					if(!empty($queryAddon))
							...
							$queryAddon .= " WHERE city = '$city'";
				}
			}
			
			$query = "SELECT * FROM gallery".$queryAddon;
				...
				<div class="product-item">
					<div class="product-thumb">
						...
						<div class="preview-meta">
							<a href="product-single.php?id=<?=$row['id']?>">View in detail</a>
                      	</div>
					</div>
...
<script type="text/javascript">
	$(document).ready(function()
	{
		$('#cityFilter').on('change',function()
		{
			...
			var cCat = "<?=!empty($_GET['cat']) ? $_GET['cat'] : ''?>";
			var val = $(this).val();
			if( val != '')
			{
